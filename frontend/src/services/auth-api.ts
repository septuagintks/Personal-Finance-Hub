...
import { http, registerRefreshHandler, setAccessToken, skipRefresh } from './http';
...
export async function registerWeb(payload: RegisterRequest): Promise<WebRegisterResponse> {
  try {
    const response = await http.post<
      operations['registerWebUser']['responses'][201]['content']['application/json']
    >('/api/v1/web/auth/register', payload, skipRefresh());
...
export async function loginWeb(payload: LoginRequest): Promise<WebTokenPair> {
  const response = await http.post<
    operations['loginWebUser']['responses'][200]['content']['application/json']
  >('/api/v1/web/auth/login', payload, skipRefresh());
  return readResponse(response);
}

export async function refreshWeb(): Promise<WebTokenPair> {
  const response = await http.post<
    operations['refreshWebSession']['responses'][200]['content']['application/json']
  >('/api/v1/web/auth/refresh', undefined, skipRefresh());
  return readResponse(response);
}
...
export function installRefreshHandler(): void {
  registerRefreshHandler(() =>
    serializeRefresh(async () => {
      const response = await refreshWeb();
