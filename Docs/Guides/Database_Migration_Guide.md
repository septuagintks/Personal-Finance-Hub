...
`role-init` 从容器环境读取运行角色凭据，不把密码放入 `psql` argv；在修改角色前拒绝 request/background 重名、当前迁移管理员复用、管理型已有角色和已有角色成员关系。request role 默认可写且对 `flyway_schema_history` 只有 `SELECT`；background role 默认只读，并在授予两组列级读取前清除历史表、序列和函数直授权。预检失败必须修正配置，不能绕过脚本手工提升角色。
