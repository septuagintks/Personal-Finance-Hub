...
| V10 | `V10__operations_and_roles.sql` | 应用角色、Operator Audit/重试事实与受限幂等清理函数 |

V1-V10 的空库迁移、重复执行、角色授权和结构门禁已通过 [Phase 2 交付矩阵](../Archive/Phase_2_S09-S12_Delivery_Summary.md)；V7-V9 租户表由 `role-init` 纳入运行角色权限和 11 张 FORCE RLS 租户表断言，V10 只向 request role 授予两个有界维护函数的执行权限。

`role-init` 从容器环境读取运行角色凭据，不把密码放入 `psql` argv；在修改角色前拒绝 request/background 重名、当前迁移管理员复用、管理型已有角色，以及两个运行角色任一方向的已有成员关系。request role 默认可写且对 `flyway_schema_history` 只有 `SELECT`；background role 默认只读，并在授予两组列级读取前清除历史表、序列和函数直授权。预检失败必须修正配置，不能绕过脚本手工提升角色。
...
`app` 只有在 Flyway 与 role-init 成功后才启动，并且只向 Compose 内网暴露 `8080`；宿主流量统一从 `web` 的同源入口进入。
