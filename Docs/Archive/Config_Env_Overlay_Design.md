...
实现 overlay 前，`JsonConfigLoader` 只读 JSON 文件，曾存在以下问题：
...
3. **向后兼容**：若无环境变量，行为与原 JSON-only 逻辑一致。
...
- [x] `JsonConfigLoader` 增加 `apply_env_overrides`，覆盖 3.1 节关键字段。
- [x] 环境变量读取支持 `PFH_*` 名称，并兼容既有无前缀名称。
- [x] 校验逻辑（拒绝占位符密钥）在 env overlay 之后执行。
- [x] 单元测试覆盖 JSON 回退、env 覆盖、占位符校验、非法端口和超范围端口。
- [x] `config/README.md` 补充环境变量使用说明。
- [x] 启动日志只输出运行环境等非敏感状态，不输出密钥值。
