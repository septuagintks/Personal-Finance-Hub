...
报表读路径由 Application 层 `ReportQueryService` 统一承载。`FinanceApplicationService` 从认证上下文取得 `UserId`，创建 request-scoped `IRequestScope`，再使用 Account、Transaction、UserPreference、ExchangeRate 与 Category Repository 读取事实并组装 DTO。PostgreSQL request scope 另提供专用 `ICashFlowProjection`，用于有界月度聚合；该读模型不进入 Domain，也不改变 In-Memory fallback 的业务语义。
...
7. PostgreSQL 在任何汇率关联前最多读取 100,001 行以判断 100,000 行上限，并校验 64 MiB 源数据预算；通过准入后才按用户时区分桶，最多返回 120 个 observed month bucket。
...
- 当前采用有界同步导出：最多读取 10,000 行和 64 MiB 输入，并在构建过程中严格限制实际 CSV 为 32 MiB。超过任一预算返回 `422 RESOURCE_LIMIT_EXCEEDED`，不会同时保留无界输入与输出。现有响应上限内不引入 chunked transport；未来若提高容量，应改为异步或流式契约后再调整。
...
现金流使用流水发生时刻，净资产使用注入的当前时刻。请求级缓存只批取当前有界页实际需要的 `(base, target, business_time)` 点，单批最多 1,024 点，同时缓存命中与缺失结果；不得加载时间窗口内的完整小时快照，Transfer 也不触发预取。

PostgreSQL 现金流投影必须与 `Decimal` 保持相同的 scale-10 Half-Even 运算边界：直接乘法和反向除法各舍入一次；USD 三角换算先舍入交叉汇率，再舍入金额。舍入后为零或不符合 `NUMERIC(20,10)` 的交叉汇率视为不可用，不得继续计算。数据库缺失汇率时不得使用 `0`、`1`、未来快照或前端默认值。
...
- PostgreSQL 现金流按用户时区在数据库侧聚合；无该投影的测试/替代 adapter 使用 200 行 keyset 页和相同总预算。
- 历史净值和跨账户余额使用批量 read model；转账页一次批取 group 明细，不允许按项目或账户形成 `N+1`。
- 分析 breakdown 在遍历有界页时增量聚合，最多 10,000 个 bucket 和 100,000 次标签展开。
- CSV 使用 10,000 行、64 MiB 输入和 32 MiB 实际输出三重上限。
- Operations summary 与 Prometheus endpoint 公开报表固定容量以及 report/CSV 超限计数。

后续性能调整必须由真实数据量、`EXPLAIN (ANALYZE, BUFFERS)` 和 RSS 测量驱动，并保持金融语义不变。
...
10. CSV 用户时区、业务金额、RFC 4180、公式注入、范围、行数、输入字节与输出字节上限。
11. PostgreSQL 投影的空窗口、Transfer-only 窗口、缺失/无效交叉汇率、scale-10 两段 Half-Even 与准入上限。
