...
- **HTTP 方法**：`GET`
- **路径**：`/api/v1/accounts/{accountId}/balance`
- **响应负载 (200 OK)**：
...
- **HTTP 方法**：`POST`
- **路径**：`/api/v1/accounts/{accountId}/archive`
- **响应负载**：`204 No Content`
...
请求时间可携带 RFC 3339 offset；所有响应时间统一规范化为 UTC `Z`，不回显
客户端原始 offset。非零小数秒必须保留并裁剪尾随零，整秒不额外输出小数部分。
