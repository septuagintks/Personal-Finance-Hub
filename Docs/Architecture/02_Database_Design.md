...
`user_preferences`、`accounts`、`categories`、`transactions`、`transfer_groups`、`transaction_tags`、`transaction_tag_relations` 与 `account_balance_cache` 启用并强制 RLS。
...
`request_idempotency` 按 `(user_id, operation, idempotency_key)` 保存金融写请求的 SHA-256 指纹、状态、过期时间和受控响应快照：
...
`users.username` 唯一，`password_hash` 使用 Argon2id PHC 字符串。`base_currency_code` 引用 `currencies(code)`，用于偏好缺失时的兼容读取。`categories_initialized` 只在注册默认数据事务完成后设为 true。
...
Transfer 必须原子创建。危险账户删除必须先删除所有触及账户的完整聚合，不能留下另一侧孤立流水。
...
- `source_version` 等于未删除流水的 `MAX(version)`。
- `last_transaction_id` 等于未删除流水的 `MAX(id)`。
- 两者同时匹配才允许命中。
- 重建时按 `transaction_time, id` 读取流水并原子 UPSERT。
...
`audit_logs.actor_type` 区分 user 与 system：user actor 必须有 `operator_user_id`，system actor 必须为空。危险操作的业务审计与业务事实同事务；汇率与 dead-letter 补充审计由幂等 Handler 写入。
...
- background role 是独立 non-superuser、BYPASSRLS、默认只读，只可查询 `users` 与 `accounts` 的活跃币种事实。
...
V1-V7 的迁移、角色授权和重复执行由部署门禁验证；真实 PostgreSQL 结果以当前 Phase 2 交付摘要为准。操作流程见 [Database Migration Guide](../Guides/Database_Migration_Guide.md)。
