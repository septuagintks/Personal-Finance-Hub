...
### 4.4 Foreign Key Delete Strategy
...
1. `transactions.amount` 使用带符号金额。
2. `income` 通常为正数，`expense` 通常为负数。
3. `transfer` 出账端为负数，入账端为正数，二者通过 `transfer_group_id` 绑定。
4. `adjustment` 按业务含义可正可负，例如手续费为负、返利为正。
...
2. **自愈逻辑**：
   ...
   ```sql
   -- 1. 计算真实的流水聚合余额
   SELECT COALESCE(SUM(amount), 0) AS real_balance,
          MAX(id) AS max_tx_id,
...
### 11.1 Exchange Rate Storage Policy
...
### 23.8 V2\_\_add_user_preferences.sql
...
### 23.9 V3\_\_add_refresh_tokens.sql
