...
private:
    ...
    static constexpr int32_t SCALE = 8; // 统一标度
};
```

示例：

- 12.34 标度为 8 变为 1234000000
- 0.0001 标度为 8 变为 10000

#### 3.2.1 精度边界与舍入规则

1. **精度边界**：
   * 系统统一采用 `int64_t` 配合固定标度 $10^8$（即小数点后保留 8 位）来实现 `Decimal`。
   * **最大表示范围**：由于 `int64_t` 的最大值为 $9,223,372,036,854,775,807$，在标度为 8 的情况下，系统最大可表示的金额约为 **922 亿**（精确值为 $92,233,720,368.54775807$）。这对于个人财务管理系统（PFH）而言完全足够，且在性能和内存占用上达到最优。
2. **银行家舍入法（Half-Even Rounding）**：
   * 系统默认且唯一采用**银行家舍入法（四舍六入五成双 / Round Half to Even）**作为舍入规则。
   * **规则定义**：当舍入位小于 5 时直接舍去，大于 5 时进位；当舍入位恰好等于 5 时，若 5 后有非零数字则进位，若 5 后无其他数字，则舍入到最接近的**偶数**。
   * **优势**：相比于传统的四舍五入，银行家舍入法在大量累加计算时能够使舍入误差相互抵消，保持统计学上的无偏性，是金融系统的标准实践。
