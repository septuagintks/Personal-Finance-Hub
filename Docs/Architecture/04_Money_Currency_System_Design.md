...
{
private:
    int64_t value;   // 缩放后的整数，固定标度为 8
    static constexpr int32_t SCALE = 8;
};
```

**边界与精度说明：**
* **底层存储与精度**：底层使用 `int64_t` 配合 $10^8$ 的固定缩放（即标度为 8）。
* **数值边界**：`int64_t` 的最大值为 $9,223,372,036,854,775,807$。在标度为 8 的情况下，系统最大可表示的金额约为 **922 亿**（精确值为 $92,233,720,368.54775807$）。这对于个人财务管理系统（PFH）而言完全足够。如果未来需要支持万亿级或超大数额，系统将平滑升级为 `__int128_t` 或引入第三方高精度库（如 `boost::multiprecision`）。
* **数据库对齐**：数据库端统一采用 `NUMERIC(20,8)`，其最大可表示金额为 $999,999,999,999.9999,9999$（千亿级）。C++ 端的 922 亿边界完全落在数据库的安全范围内。
...
### 3.3.1 默认舍入规则：银行家舍入法 (Half-Even Rounding)

为了在频繁的汇率折算、利息计算和报表汇总中避免累积误差，系统**统一采用银行家舍入法（四舍六入五双 / Round Half to Even）**作为默认的舍入规则。
...
* **规则定义**：
  ...
  * 当舍弃位恰好等于 5 时：
    * 若 5 后面还有非零数字，则进位。
    * 若 5 后面全是零，则看 5 前面的数字：如果是奇数则进位（凑成偶数），如果是偶数则舍去（保持偶数）。
* **示例（保留 2 位小数）**：
  * `1.124` -> `1.12`（小于 5 舍去）
  * `1.126` -> `1.13`（大于 5 进位）
  * `1.125` -> `1.12`（5 后无数字，前位 2 是偶数，舍去）
  * `1.135` -> `1.14`（5 后无数字，前位 3 是奇数，进位）
  * `1.12501` -> `1.13`（5 后有非零数字，进位）
