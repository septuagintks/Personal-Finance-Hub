...
{
private:
    int64_t value;   // 缩放后的整数
    int32_t scale;   // 小数点后位数
};
```
...
2. **舍入规则（Rounding Mode）**：金融计算中，除法和乘法（如汇率折算）会产生超出 8 位小数的数值。系统**统一采用银行家舍入法（Half-Even Rounding / Round Half to Even）**作为默认的舍入规则，而不是简单的四舍五入，以减小累计误差。
3. **除法约束**：除法必须显式定义舍入模式。
4. **标度不匹配**：标度不匹配必须确定性处理。
5. **负值支持**：必须允许负值，因为存在冲销和更正。
6. **数值边界**：
   * 数据库中金额字段定义为 `NUMERIC(20,8)`，最大可表示 $999,999,999,999.99999999$。
   * C++ 的 `Decimal` 底层如果使用 `int64_t` 配合 $10^8$ 的缩放，其最大值约为 $92,233,720,368.54775807$（约 922 亿）。
   * 对于个人财务系统，922 亿的上限完全足够。因此，**第一阶段允许 `Decimal` 底层使用 `int64_t`**。若未来需要支持企业级账务或津巴布韦币等极端通胀货币，可无缝替换为 `__int128_t` 或 `boost::multiprecision`，而无需修改上层业务代码。
