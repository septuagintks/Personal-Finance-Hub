...
6. **数值边界**：
   ...
   - Repository 在写入前必须重复执行 `NUMERIC(20,8/10)` 边界检查，作为绕过应用层时的纵深防御。
   ...
   - `Decimal` 的有效 raw storage 范围固定为 $[-2^{126}, 2^{126}]$。所有公开构造与算术出口必须检查该不变量；内部 magnitude 计算只在 unsigned domain 完成，因此任意有效值的 `abs()` 与 `negated()` 都不会触发 signed overflow。
...
1. 货币代码必须属于项目当前受控支持的 ISO-4217 法币子集，或属于批准的加密货币白名单
2. 货币对象应该不可变
3. 货币比较必须精确
...
默认舍入策略统一为 `银行家舍入`（Half-Even）。其他策略仅在具体业务场景显式记录时使用，避免同一类计算在不同模块产生不可重现的结果。
...
- 在 UI 中优先使用账户余额快照，不直接用原始流水在前端重算余额
- 当报告日期是历史日期时使用历史汇率
- 根据报表设置包括所有活跃和已归档的账户
