...
Release Candidate 必须使用仓库固定工具验证资源上限：

| 门禁 | 固定范围与预算 |
| ---- | ------------ |
| Sanitizer | Linux GCC/Clang Debug 分别执行 ASan 与 UBSan；默认开启 leak detection、首个错误即停止 |
| Daily | 10,000 条流水；transaction 300 ms、transfer 400 ms、Dashboard 500 ms、120 月现金流 1,000 ms、标签报表 1,500 ms、CSV first-byte 2,000 ms p95 |
| Stress | 100,000 条流水；transaction 500 ms、transfer 800 ms、Dashboard 800 ms、120 月现金流 3,000 ms、标签报表 4,000 ms、CSV first-byte 5,000 ms p95 |
| PostgreSQL plan | fixture 数量必须精确匹配；列表关键路径不得对 `transactions` 做未接受的顺序扫描；执行时间不得超过对应 profile 预算 |
| API pressure | 非预期状态与超过 5% 的普通请求传输失败均失败；auth/queue 必须有对应 Operator 拒绝计数增长；10 秒内恢复；峰值 RSS 增长不超过 256 MiB，冷却后增长不超过 96 MiB |
| Browser session | 流水最多驻留 4 页/200 条；Reports 保持 2 个 canvas；预热后 20 轮路由往返的强制 GC heap 增长不超过 16 MiB |

`tools/phase2_performance.py`、`tools/phase2_pressure.py` 与 `tools/run_sanitizers.sh` 是上述门禁的执行入口。数据库 fixture 只能写入可删除的测试库；URL、Token、Provider key、完整计划和原始压力日志不得提交。Chromium CDP 用例验证 heap；Firefox/WebKit 仍执行功能、Accessibility 与生命周期回归，但不伪造 CDP 结果。具体命令见 [Linux Development Workflow](../Guides/Linux_Development_Workflow.md)。
