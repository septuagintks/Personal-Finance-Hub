...
Status: In Progress; P2-S01/P2-S02/P2-S03 Complete; P2-S04 Implementation Complete, Target-Environment Validation Pending
...
状态：Complete。OpenAPI 2.0、30 个唯一 operationId、Web Cookie 会话、金融写幂等、结构化错误、强 ETag 解析和生成类型 drift 门禁已落地。跨标签页 single-flight 属于 S03/S04 前端运行时验收。
...
状态：Implementation Complete；Target-Environment Validation Pending。本机会话 Store、路由守卫、注册/登录/退出、内存 Access Token、single-flight refresh、安全请求重放、跨标签页互斥和仅状态广播已实现。用户偏好与公共货币元数据会在认证状态就绪前并行预载，并以 `AbortController` 和 generation 隔离退出、重登及迟到响应；当前尚未实现的默认首页统一回退 `/dashboard`。
...
状态：Implementation Complete；Target-Environment Validation Pending。本机已完成账户状态查询、详情与余额组合、强 ETag 修改、币种历史冻结、归档/恢复、三阶段危险删除、用户状态隔离及 Desktop/Mobile 浏览器闭环。真实 Drogon/PostgreSQL、RLS 连接复用和 Linux 多浏览器验收保留到 P2-S12。
...
- Vitest/MSW `28/28 PASS`，覆盖账户 API validator、固定三次确认、详情/余额原子发布、用户切换竞态、迟到写响应隔离和 Logout 清理。
