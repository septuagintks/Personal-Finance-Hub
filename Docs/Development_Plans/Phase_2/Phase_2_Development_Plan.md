...
Status: In Progress; P2-S01-P2-S11 Complete, P2-S12 Target-Environment Revalidation Pending
...
- Web Edge 改为 lockfile 冷构建，Nginx 同源代理仅在 Compose 内网暴露的 `app` 服务，并设置 CSP、clickjacking、MIME、Referrer、Permissions、COOP 与缓存策略；Backend healthcheck 使用队列无关 `/livez`，Compose 中 Web/API 均为 non-root、read-only、drop capabilities 和 no-new-privileges。
...
状态：Windows Remediation Complete（Target-Environment Revalidation Pending）。当前候选 `b70c41a` 已通过 Windows Debug/Release `381/381`、Vitest/MSW `63/63`、Edge E2E `37/37` 与发布门禁；较早候选的 Linux production ON、三浏览器和镜像门禁已通过。V8/V9 及发布边界修复后的 PostgreSQL 16+、完整 Docker runtime、Provider、固定性能、备份恢复和回滚仍须在目标侧重跑，Phase 尚未签署。
