...
* **示例**：
  * *不规范*：`C++23的强类型ID在Drogon框架中被广泛使用。`
  * *规范*：`C++23 的强类型 ID 在 Drogon 框架中被广泛使用。`
...
* **示例**：
  * *不规范*：`在 Domain 层,我们定义了 IAccountRepository; 它的实现位于 Infrastructure 层.`
  * *规范*：`在 Domain 层，我们定义了 IAccountRepository；它的实现位于 Infrastructure 层。`
